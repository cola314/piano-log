          ...
          <div style={{ display: "flex", gap: "8px" }}>
            {CATEGORIES.map((cat) => (
              <button key={cat.id} onClick={() => handleCategoryChange(cat.id)} style={{
                ...
                background: category === cat.id ? `${cat.color}20` : "rgba(0,0,0,0.2)",
                color: category === cat.id ? "#F5E6C8" : "#6B5B45",
              ...
              {hanonStep === 0 ? (
                <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                  {KEYS.map((k) => (
                    <button key={k} onClick={() => { setHanonKey(k); setHanonStep(1); }} style={{
                      ...
                      border: "1px solid rgba(184,134,11,0.15)", background: "rgba(0,0,0,0.2)",
                ...
                <>
                  <button onClick={() => setHanonStep(0)} style={{
                    background: "none", border: "none", color: "#6B5B45",
                  ...
                  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                    {SCALE_TYPES.map((st) => (
                      <button key={st} onClick={() => setTitle(`${hanonKey} ${st}`)} style={{
                        ...
                        background: title === `${hanonKey} ${st}` ? "rgba(184,134,11,0.2)" : "rgba(0,0,0,0.2)",
